...
const CustomDropdown: React.FC<CustomDropdownProps> = ({
  ...
  const handleSelectOption = (option: Option) => {
    setSelected(option);
    setIsOpen(false);
    // Notifica o pai que selecionamos esse option
    if (onSelectOption) {
  ...
  return (
    <div className="flex flex-col space-y-1 w-64" ref={dropdownRef}>
      ...
      <button
        ...
        className="relative flex justify-between items-center px-4 py-2 border border-gray-300 rounded-full text-left focus:outline-none focus:ring-2 focus:ring-blue-500"
      >
      ...
      {isOpen && (
        <div className="relative">
          <div className="absolute z-10 w-full bg-white border border-gray-200 rounded-md shadow-lg mt-1">
            {/* Primeiro item com + para adicionar */}
            <div
            ...
            {options.map((option) => (
              ...
              >
                {/* Rótulo do item */}
                <button
                  className="text-gray-700 text-left w-full"
                ...
                {/* Ícone 3 pontinhos */}
                <div className="relative">
                  <button className="p-1 ml-2 text-gray-500 hover:text-gray-700 focus:outline-none">
                    <FaEllipsisV />
                  </button>
                  {/* Menu ao clicar nos 3 pontinhos (aparece no hover via "group-hover:block" ou pode ser sempre aberto) */}
